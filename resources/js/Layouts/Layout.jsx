...
export default function Layout({ children }) {
    ...
    return (
        <div className="flex h-screen">
            ...
            {/* Side Navigation Bar */}
            <div className={`bg-gray-900 text-white p-6 space-y-6 ${isCollapsed ? 'w-20' : 'w-64'} transition-all duration-300`}>
                ...
                <ul className="space-y-4 mt-6">
                    <li>
                        <Link className={`flex items-center space-x-3 active-link hover:text-primary ${isCollapsed ? 'justify-center' : ''} ${isActive('/')}`} href="/">
                            <FaHome />
                            {!isCollapsed && <span>Dashboard</span>}
                        </Link>
                    </li>
                    <li>
                        <Link className={`flex items-center space-x-3 hover:text-primary ${isCollapsed ? 'justify-center' : ''} ${isActive('/reports')}`} href="#">
                            <FaFile />
                            {!isCollapsed && <span>Reports</span>}
                        </Link>
                    </li>
                    <li>
                        <Link className={`flex items-center space-x-3 hover:text-primary ${isCollapsed ? 'justify-center' : ''} ${isActive('/settings')}`} href="#">
                            <FaGear />
